...
async fn subscribe_returns_a_200_for_valid_form_data() {
    ...
    let response = client
        ...
        .body(body)
        .send()
        .await.expect("Fail to execute request.");
    assert_eq!(200, response.status().as_u16());
}

#[tokio::test]
async fn subscribe_return_a_415_when_data_is_missing() {
    let app_address = spawn_app();
    let client = reqwest::Client::new();
    // table-driven-test
    let test_cases = vec![
    ...
    for (invalid_body, error_message) in test_cases {
        let response = client
            ...
            .header("Content-Type", "application/x-www-form-unlencoded")
            .body(invalid_body)
            .send().await.expect("Fail to execute request.");
        assert_eq!(
            415, response.status().as_u16(), "The API did not fail with 400 Bad Request when the payload was {}.", error_message);
    }
}